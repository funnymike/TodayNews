...
struct UserDetail: HandyJSON {
    ...
    var descriptionHeight: CGFloat { return Calculate.textHeight(text: description, fontSize: 13, width: kScreenWidth - 30.0) }
...
struct UserDetailDongtai: HandyJSON {
    ...
    var collectionViewW: CGFloat {
        return isDongtaiDetail ? (kScreenWidth - 30) : Calculate.collectionViewWidth(thumb_image_list.count)
    }
    ...
    var detailContentH: CGFloat {
        return Calculate.textHeight(text: content, fontSize: 17, width: kScreenWidth - 30.0)
    }
    
    var contentH: CGFloat {
        let height = Calculate.textHeight(text: content, fontSize: 17, width: kScreenWidth - 30.0)
        return height >= 110 ? 110 : height
    }
    
    /// 富文本内容高度
    var attributedCntentHeight: CGFloat {
        let height = Calculate.attributedTextHeight(text: attributedContent, width: kScreenWidth - 30.0)
...
struct DongtaiOriginThread: HandyJSON {
    ...
    var contentH: CGFloat {
        ...
        let height = Calculate.textHeight(text: nameAndContent, fontSize: 17, width: kScreenWidth - 30.0) + 5.0
...
struct UserDetailQuestion: HandyJSON {
    ...
    var title: String = ""
    var titleH: CGFloat {
        return Calculate.textHeight(text: title, fontSize: 17, width: kScreenWidth - 30.0) + 5
    }
...
struct UserDetailAnswerContentAbstract: HandyJSON {
    ...
    var thumb_image_list: [ThumbImage]! = [ThumbImage]()
    var text: String = ""
    var textHeight: CGFloat {
        let height = Calculate.textHeight(text: text, fontSize: 16, width: kScreenWidth - 30.0)
        return height >= 42 ? 42 : height
    }

    var large_image_list: [LargeImage]! = [LargeImage]()
}
