        ...
        didSet {
            ...
            if aNews.video_duration != 0 && aNews.has_video { // 有视频
                if aNews.video_style == 0 {  // 右侧有图
                    ...
                    rightImageViewWidth.constant = kScreenWidth * 0.28
                } else if aNews.video_style == 2 { // 大图
                    middleViewHeight.constant = kScreenWidth * 0.5
            ...
            // 没有视频
                ...
                if aNews.middle_image.url != "" && aNews.image_list.count == 0 {
                    ...
                    if aNews.image_list.count == 1 { // 右侧显示图片
                        ...
                        collectionView.frame = CGRect(x: 0, y: 0, width: kScreenWidth - 30, height: image3Width)
                        collectionView.images = aNews.image_list
                    }
                }
            }
            layoutIfNeeded()
            videoImageButton.frame = CGRect(x: 0, y: 0, width: middleView.width, height: kScreenWidth * 0.5)
        }
    }
